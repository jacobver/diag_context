...
class Translator(object):
    def __init__(self, opt):
        ...
        self.attn = onmt.modules.GlobalAttention(model.enc.rnn_sz[0])
    ...
    def translateBatch(self, srcBatch, tgtBatch):
        ...
        encStates, context = self.model.encoder(srcBatch[0])
        ...
        if tgtBatch is not None:
            decStates = encStates
            decOut = self.model.make_init_decoder_output(encStates[0][0])
            mask(padMask)
            initOutput = self.model.make_init_decoder_output(encStates[0][0])
        ...
        decStates = ((Variable(encStates[0][0].data.repeat(1, beamSize, 1)),
                      Variable(encStates[0][1].data.repeat(1, beamSize, 1))),
                     (Variable(encStates[1][0].data.repeat(1, beamSize, 1)),
                      Variable(encStates[1][1].data.repeat(1, beamSize, 1))))

        beam = [onmt.Beam(beamSize, self.opt.cuda) for k in range(batchSize)]

        decOut = self.model.make_init_decoder_output(decStates[0][0])
